...
def clean_dataframe(df):
    """Bereinigt und normalisiert DataFrame - ROBUST für beide Umlaut-Schreibweisen"""
    if df.empty:
        return df
    ...
    # ROBUSTE Dimensionen bereinigen - beide Schreibweisen unterstützen
    dimension_mappings = {
        "Breite": ["Breite"],
        "Höhe": ["Höhe", "Hoehe"],  # Beide Varianten
        "Zoll": ["Zoll"]
    }
    ...
    for standard_name, variants in dimension_mappings.items():
        for variant in variants:
            if variant in df.columns:
                # Spalte umbenennen falls sie nicht schon den Standard-Namen hat
                if variant != standard_name:
                    df[standard_name] = df[variant]
                
                # Numerisch konvertieren
                df[standard_name] = pd.to_numeric(df[standard_name], errors="coerce").astype("Int64")
                break
    ...
    # Nur Zeilen entfernen die in ALLEN wichtigen Spalten NaN sind
    essential_cols = ["Preis_EUR", "Breite", "Höhe", "Zoll"]
...
def add_to_cart_with_config(tire_data, quantity, services):
    ...
    cart_item = {
        'id': tire_id,
        'Reifengröße': f"{tire_data['Breite']}/{tire_data['Höhe']} R{tire_data['Zoll']}",
...
def render_tire_list(filtered_df):
    ...
    display["Reifengröße"] = (
        display["Breite"].astype(str) + "/" + display["Höhe"].astype(str) + " R" + display["Zoll"].astype(str)
    )
...
def render_statistics(filtered_df):
    ...
    with col4:
        unique_sizes = len(filtered_df[["Breite", "Höhe", "Zoll"]].drop_duplicates())
...
def main():
    ...
    # DEBUG: Zeige Anzahl geladener Reifen
    ...
    with st.sidebar:
        ...
        höhe_opt = ["Alle"] + sorted(df["Höhe"].unique().tolist())
        ...
        höhe_filter = st.selectbox("Höhe (%)", options=höhe_opt, index=0)
    ...
    if st.session_state.selected_size:
        ...
        filtered = filtered[(filtered["Breite"] == b) & (filtered["Höhe"] == h) & (filtered["Zoll"] == z)]
    ...
    if höhe_filter != "Alle":
        filtered = filtered[filtered["Höhe"] == int(höhe_filter)]
    if fabrikat != "Alle":
    ...
    if sortierung == "Preis aufsteigend":
        ...
        filtered = filtered.sort_values(["Zoll", "Breite", "Höhe", "Preis_EUR"])
