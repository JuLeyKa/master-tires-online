...
<style>
    ...
    .saison-winter   { background-color: #dbeafe; color: #1e40af; }
    .saison-sommer   { background-color: #fef3c7; color: #92400e; }
    .saison-ganzjahres { background-color: #d1fae5; color: #065f46; }
    ...
    .logo-container {
        ...
        padding: 1.9rem 0 0.5rem 0; margin: 0;
    }
...
def get_dynamic_tire_sizes(filtered_df, max_sizes=12):
    ...
    unique_sizes['size_str'] = (
        unique_sizes['Breite'].astype(str) + '/' +
        unique_sizes['Hoehe'].astype(str) + ' R' +
        unique_sizes['Zoll'].astype(str)
    )
    return unique_sizes.head(min(max_sizes, len(unique_sizes)))['size_str'].tolist()
...
def create_metric_card(title, value, delta=None, help_text=None):
    ...
    return f"""
    <div style="background:var(--background-white);border:1px solid var(--border-color);border-radius:var(--border-radius);padding:1rem;box-shadow:var(--shadow-sm);transition:transform 0.2s ease;"
...
def clean_dataframe(df):
    ...
    required_cols = ["Fabrikat","Profil","Kraftstoffeffizienz","Nasshaftung","Loadindex","Speedindex","Teilenummer","Geräuschklasse"]
...
# ================================================================================================
SERVICE_REQUIRED_COLUMNS = {"Positionsnummer","Bezeichnung","Teilenummer","Detail","Preis","Hinweis","Kategorie","Zoll","Anzahl"}

def _validate_service_df(df: pd.DataFrame) -> bool:
    cols = set(df.columns)
    missing = SERVICE_REQUIRED_COLUMNS - cols
    if missing:
...
def _normalize_service_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Trim & normalize text cols
    for c in ["Kategorie","Zoll","Bezeichnung","Detail","Hinweis","Positionsnummer","Teilenummer"]:
        if c in df.columns:
            df[c] = df[c].astype(str).str.strip()
    # Normalize keys used for filtering
    df["Kategorie"] = df["Kategorie"].str.lower()
    df["Zoll"]      = df["Zoll"].str.lower()
    # Numeric casts
    if "Anzahl" in df.columns:
        df["Anzahl"] = pd.to_numeric(df["Anzahl"], errors="coerce").astype("Int64")
    if "Preis" in df.columns:
        # Entferne evtl. € / ,- aus Versehen
...
def load_service_packages() -> pd.DataFrame:
    ...
    # Wenn CSV fehlt → Default-Pakete bauen
    if not csv_path.exists():
        return create_default_service_packages()

    # Robust lesen: Dialekt-Erkennung (Komma / Semikolon), UTF-8-SIG
    try:
        df = pd.read_csv(csv_path, sep=None, engine='python', encoding='utf-8-sig')
    except Exception:
        # Fallback: versuche Komma, dann Semikolon
        try:
    ...
    if not _validate_service_df(df):
        st.info("Standard-Pakete werden verwendet (ungültige services_config.csv).")
        return create_default_service_packages()
    return df
...
def get_reifenservice_package(zoll_size: int, anzahl: int):
    packages = get_service_packages()
    zoll_cat = _zoll_cat_from_size(zoll_size)
    try:
        subset = packages[
            (packages['Kategorie'] == 'reifenservice') &
            (packages['Zoll'] == zoll_cat) &
            (packages['Anzahl'].astype('Int64') == int(anzahl))
        ]
    except KeyError:
        # Fehlende Spalten -> Fallback
        return None
    if not subset.empty:
        return subset.iloc[0]
    return None

def get_auswuchten_package(anzahl: int):
    packages = get_service_packages()
    try:
        subset = packages[
...
def get_raederwechsel_package(anzahl: int):
    packages = get_service_packages()
    try:
        subset = packages[
...
def get_kombi_package(typ: str):
    packages = get_service_packages()
    kategorie = 'kombi_komfort' if typ == 'komfort' else 'kombi_standard'
    try:
        subset = packages[packages['Kategorie'] == kategorie]
    except KeyError:
        return None
    if not subset.empty:
        return subset.iloc[0]
...
# ================================================================================================
# RENDER FUNCTIONS – MIT SERVICE-PAKETEN
# ================================================================================================
def render_config_card(row, idx, filtered_df):
    ...
    with col_config1:
        quantity = st.number_input(
            "Stückzahl:", min_value=1, max_value=8, value=4, step=1,
            key=f"qty_{idx}", help="Anzahl der Reifen (1-8 Stück)"
        )
    ...
    with col_config2:
        ...
        # Nur Auswuchten (wenn kein Reifenservice)
        ...
        if raederwechsel_selected:
            with st.expander("Räderwechsel-Optionen", expanded=True):
                raederwechsel_options = [(4,"4 Räder (Standard)"),(2,"2 Räder"),(1,"1 Rad")]
        ...
        # Kostenberechnung via Pakete
        service_total = 0.0
        service_details = []

        if reifenservice_selected:
            package = get_reifenservice_package(zoll_size, quantity)
            if package is not None and pd.notna(package.get('Preis', np.nan)):
                service_total += float(package['Preis'])
...
def render_legend(mit_bestand, saison_filter, zoll_filter):
    ...
    with col2:
        ...
        filter_info = []
        if mit_bestand: filter_info.append("Bestandsfilter aktiv")
        if saison_filter != "Alle": filter_info.append(f"Saison: {saison_filter}")
        if zoll_filter != "Alle": filter_info.append(f"Zoll: {zoll_filter}")
        if filter_info:
...
def main():
    ...
    with col2:
        ...
        st.selectbox("Saison-Typ:", options=saison_options, key="top_saison_filter", help="Filtere nach Reifen-Saison basierend auf Teilenummer")
    with col3:
        zoll_opt = ["Alle"] + sorted(df["Zoll"].unique().tolist())
        st.selectbox("Zoll:", options=zoll_opt, key="top_zoll_filter", help="Filtere nach Zoll-Größe")
    with col4:
    ...
    if len(filtered) > 0:
        ...
        if mit_bestand: fi.append("mit Bestand")
        if saison_filter != "Alle": fi.append(f"({saison_filter})")
        if zoll_filter != "Alle": fi.append(f"{zoll_filter} Zoll")
        header_text = f"Gefundene Reifen: {len(filtered)}" + (f" {' '.join(fi)}" if fi else "")
        ...
        if mit_bestand: fi.append("mit Bestand")
        if saison_filter != "Alle": fi.append(f"Saison: {saison_filter}")
        if zoll_filter != "Alle": fi.append(f"Zoll: {zoll_filter}")
        if fi:
            st.warning(f"Keine Reifen {' und '.join(fi)} gefunden. Bitte Filter anpassen.")
        else:
